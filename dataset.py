...
def clean_entry(content):
    """remove duplicate entries generated by .yaml()"""
    keyword = 'sources:'
    keys = content.split('\n')
    if keyword in keys:
        keys.remove(keyword)
...
def build_climate_catalog(filename=paths.CLIMATE_CATALOG):
    ...
    catalog_content = []
    catalog_content.append(build_yearly_temperature_entry(OLDEST_DATE, NOW, 'historical', '0'))
    for rcp in ['4.5', '8.5']:
        catalog_content.append(build_yearly_temperature_entry(NOW, FURTHER_DATE, f'prediction RCP {rcp}', rcp))
